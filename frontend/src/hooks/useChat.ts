...
export function useChat() {
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      // Image bytes travel with the turn, but it is the agent — following the
      // visitor's words — that decides whether to wake the VLM Space. Nothing
      // on this side reads the artwork on its own.
      // Clear the error FIRST: the store's setError(null) also resets status to
      // "idle", so calling it after setStatus wiped the busy state in the same
      // tick and the pending bubble (feather, phrase, counter) never rendered.
      setError(null);
      ...
      try {
        ...
        addMessage({
          ...
          clevelandResults: response.cleveland_results,
          artAdvice: response.art_advice,
          galleryResults: response.gallery_results,
        });
        setStatus("idle");
