...
def _format(hit: Hit) -> str:
    ...
    Uses ``Hit.citation`` so the source reference the LLM sees is the same
    string the validation script prints — book title, real page range, and
    the chapter heading when one was detected.
    """
    return f"[Source: {hit.citation}]\n\n{hit.text}"


def query_art_history(question: str, k: int = 4) -> str:
